...
save(po, file = "data/plot_object.rda")
...
p1 <- plot_by_taxa(po, "rhizosphere", "Species")
p4 <- plot_by_taxa(po, "bulk soil", "Species")
...
plot_totab <- function(po, h, g){
  #h = "rhizosphere"
  #g = "Species"
  subset <- subset(po, Fraction == h & grp == g & mean_pc >= 5) #& mean_pc >= 5
...
p2 <- plot_totab(po, "rhizosphere", "Species")
p5 <- plot_totab(po, "bulk soil", "Species")
...
diffab <- function(ps, fraction){
  ...
  diffab <- po %>%
    ...
    subset(Fraction == tolower(fraction) & grp == "Species")
...
plot_diffab <- function(
  ...
  #subset <- subset(diffab, mean_pc >= pc_cutoff)
...
p3 <- plot_diffab(diffab_RHZ, label_neg = "more abundant in Corn", label_pos = "more abundant in Soybean")
p6 <- plot_diffab(diffab_SOL, label_neg = "more abundant in Corn", label_pos = "more abundant in Soybean")
...
g = "Species"
subset <- pp %>%
  subset(Fraction == h & grp == "Species") #!(taxa2 %in% bimod)) #%>%
  #top_n(20, mean_pc)
...
asv_selection <- as.character(subset(var, Fraction == "rhizosphere" & taxa2 == "g__Bradyrhizobium")$ASV)
taxa_subset <- prune_taxa(asv_selection, ps)
tree <- ggtree(phy_tree(taxa_subset), branch.length = "none")

matrixdf <- var %>%
  subset(Fraction == "rhizosphere" & taxa2 == "g__Bradyrhizobium") %>%
  rownames_to_column(var = "old_rownames") %>%
  select(ASV, Year:Rep) %>%
  mutate_at(c("Year", "Month", "Species", "Rotation", "Nitrogen", "Block", "Rep"), function(x){log((x-mean(x))/sd(x))}) %>%
  column_to_rownames("ASV")



gheatmap(tree, matrixdf, offset=5, width=.3,
